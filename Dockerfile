...
RUN apt-get update && apt-get install -y --no-install-recommends \
    build-essential ca-certificates curl libcurl4-openssl-dev libedit-dev libjansson-dev libsqlite3-dev \
...
RUN ./configure --prefix=/usr --sysconfdir=/etc --localstatedir=/var --with-pjproject-bundled \
 ...
 && menuselect/menuselect --disable-all \
    ...
    --enable func_callerid --enable func_curl --enable func_cut --enable func_json --enable func_uuid --enable pbx_config --enable res_pjproject --enable res_pjsip --enable res_pjsip_authenticator_digest \
    ...
    --enable res_audiosocket --enable res_curl --enable res_geolocation --enable res_statsd menuselect.makeopts \
...
RUN apt-get update && apt-get install -y --no-install-recommends \
    libcurl4 libedit2 libjansson4 libpq5 libsqlite3-0 libssl3 libxml2 libxslt1.1 libuuid1 odbc-postgresql \
